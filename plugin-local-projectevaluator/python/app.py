...
async def evaluate_submission(payload: dict):
    """Evaluate a single submission text and return suggested grade and feedback.

    Expected payload: {"text": "...submission text...", "max_grade": 100}
    """
    try:
        text = payload.get('text', '')
        max_grade = int(payload.get('max_grade', 100))

        if not text or not text.strip():
            raise HTTPException(status_code=400, detail='Empty submission text')

        prompt = (
            "You are an experienced instructor. Given the student submission below, "
            f"provide a numeric grade out of {max_grade} and a concise feedback comment (1-3 sentences).\n\n"
            "Submission:\n"
            f"{text}\n\n"
            "Respond as JSON: {\"grade\": <number>, \"feedback\": <string>}"
        )
        ...
        if not result or not result[0]:
            return JSONResponse(status_code=502, content={"error": "No response from Cortex"})
        ...
        if start != -1 and end != -1 and end > start:
            ...
            try:
                ...
                grade = float(data.get('grade', max_grade))
        ...
        # Fallback extraction
        import re as _re
        m = _re.search(r"(\d{1,3}(?:\.\d+)?)", raw)
        grade = float(m.group(1)) if m else max_grade
        feedback = raw if len(raw) < 1000 else raw[:1000]
        return { 'grade': min(grade, max_grade), 'feedback': feedback }
